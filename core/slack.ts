...
export interface CurrentSlackStatus {
	message: string;
	icon: string;
...
export interface SlackProfileDataSend {
	profile: {
		status_text: string;
		status_emoji: string;
...
export interface SlackProfileDataResponse extends SlackApiResponse, SlackProfileDataSend {}

export interface SlackDndDataSend extends SlackApiResponse {
	num_minutes: number;
}

export interface SlackDndDataResponse extends SlackApiResponse {
...
async function fetchOrSubmitJson<Sends, Receives>(uri: string, bodyToSend?: Sends): Promise<Receives> {
	try {
		...
		requestOrParseError.message = [
			'Error requesting or parsing resource',
			uri,
			requestOrParseError.message,
		].join(' ');
...
export async function publishNewStatusToSlack(scheduleItem: SelectedScheduleItem, existingStatus: CurrentSlackStatus) {
	...
	if (scheduleItem.doNotDisturb && !existingStatus.doNotDisturb) {
		...
		const result = await fetchOrSubmitJson<SlackDndDataSend, SlackDndDataResponse>(
			`https://slack.com/api/dnd.setSnooze`,
			{ num_minutes: dndDurationInMinutes },
		);
		...
		const result = await fetchOrSubmitJson<EmptyObject, SlackDndDataResponse>(
...
function loadSecret(secretId: string): Promise<string> {
	try {
